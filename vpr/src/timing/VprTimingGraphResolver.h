...
class VprTimingGraphResolver : public tatum::TimingGraphNameResolver {
  ...
  private:
    ...
    t_rt_node* find_pointer_to_sink(t_rt_node* rt_root,ClusterNetId net_id, ClusterPinId sink_pin) const;
    t_rt_node* find_pointer_to_sink_recurr(t_rt_node* node, int sink_inode) const;
